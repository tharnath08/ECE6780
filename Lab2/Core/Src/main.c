...
void EXTI0_1_IRQHandler(void){
	if (EXTI->PR & (1<<0)){
		HAL_Delay(1000);
		GPIOC->ODR ^= ((1 << 9)|(1<<8)); // Toggling PC8 & PC9
...
int main(void)
{
  HAL_Init(); // Reset of all peripherals, init the Flash and Systick
	...
	RCC->AHBENR |= RCC_AHBENR_GPIOCEN; // Enabling the GPIOC Timer
	RCC->AHBENR |= RCC_AHBENR_GPIOAEN; // Enabling the GPIOA Timer
	RCC->APB2ENR |= RCC_APB2ENR_SYSCFGEN; // Enabling the SYSCFG Timer
