...
volatile uint8_t previous_error = 0;
...
void PI_update(void) {
		output = 0;
		motor_rpm = motor_speed/2;
		
		error = target_rpm - motor_rpm;
    
		error_integral = error + previous_error;
     ...
     * 
     s* Hint: If you chose 3200 for the integral clamp you should divide by 32 (right shift by 5 bits), 
